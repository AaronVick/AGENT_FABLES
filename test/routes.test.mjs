...
test('launch audit separates complete local artifacts from external publication state', () => {
  ...
  assert.equal(result.public_git_readiness, true)
  assert.deepEqual(result.public_git_blockers, [])
  assert.equal(result.ecosystem_distribution_readiness, false)
  ...
  assert.ok(result.blockers.includes('mcp_package_published'))
