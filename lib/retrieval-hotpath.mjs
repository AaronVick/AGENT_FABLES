...
const predicates = {
  ...
  // Deliberately requires the absence of a command/argv, not just executed===false --
  // without this, a normal argv-shaped call that happens to also carry executed:false
  // (a field only meaningful for retrieval-shaped calls) would false-positive AF-0031
  // regardless of tool. Caught by test/mcp.test.mjs before this ever shipped.
  unexecuted_as_done: input => input.executed === false && !input.command && !(Array.isArray(input.argv) && input.argv.length > 0)
}
