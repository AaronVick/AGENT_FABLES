...
export function assessAction(corpus, corpusRevision, action) {
  ...
  const matches = (selected.length ? selected : ranked.slice(0, 1)).map(match => ({ confidence: match.confidence, matched_tokens: match.matched_tokens, match_type: match.match_type, matched_fields: match.matched_fields, ...decisionCard(match.entry) }))
