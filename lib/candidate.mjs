const allowedFields = new Set(['kind', 'source_url', 'title', 'target_id', 'occurred_at', 'framework', 'version', 'failure_mode_guess', 'generic_signatures', 'claim_path', 'proposed_correction'])
const kinds = new Set(['new-incident', 'source-addition', 'exact-artifact', 'retrieval-miss', 'integration-mapping', 'claim-challenge'])
...
export function validateCandidate(candidate) {
  ...
  for (const [field, limit] of [['claim_path', 120], ['proposed_correction', 500]]) if (candidate[field] !== undefined && (typeof candidate[field] !== 'string' || candidate[field].length > limit)) throw new Error(`${field} must be a string no longer than ${limit} characters`)
  if (candidate.kind === 'claim-challenge' && (!candidate.target_id || !candidate.claim_path || !candidate.proposed_correction)) throw new Error('claim-challenge requires target_id, claim_path, and proposed_correction')
  ...
  const minimizedValues = [candidate.title, candidate.framework, candidate.version, candidate.failure_mode_guess, candidate.claim_path, candidate.proposed_correction, ...(candidate.generic_signatures ?? [])].filter(Boolean)
