...
export function verifyInstallation(root) {
  const required = ['index.json', 'index.jsonl', 'memory.jsonl', 'incidents.json', 'trust.json', 'capabilities.json', 'steward.json', 'contact-policy.json', 'leaders.json', 'freshness.json', 'evidence-coverage.json', 'eval-report.json', 'metrics-report.json', 'consumer-obligations.json', 'guardrail-contract.json', 'contribution-contract.json', 'adoption-kit.json', 'publication-state.json', 'START_HERE.md', 'SANDBOX.md', 'INJECT.txt', 'hotpath.json', 'tool-index.jsonl', 'utterance-index.jsonl', 'overlaps.json', 'cite-contract.json', 'agent-entry.json', 'sandbox/agent-fables-sandbox.mjs', 'schemas/hotpath-input.schema.json', 'schemas/hotpath-receipt.schema.json', 'schemas/card.schema.json', 'schemas/guardrail-finding.schema.json', 'schemas/evidence-candidate.schema.json']
  ...
  const evalReport = JSON.parse(fs.readFileSync(path.join(root, 'eval-report.json'), 'utf8'))
  const metricsReport = JSON.parse(fs.readFileSync(path.join(root, 'metrics-report.json'), 'utf8'))
  ...
  const checks = {
    ...
    , low_capability_entry_matches_corpus: agentEntry.authority === 'none' && agentEntry.authorization === 'not-granted' &&
      agentEntry.no_match_means_safe === false && agentEntry.corpus_revision === computedRevision &&
      agentEntry.counts.patterns === corpus.length && agentEntry.counts.incidents === incidents.incident_count
    , standalone_sandbox_runtime_is_pinned: fs.readFileSync(path.join(root, 'sandbox', 'agent-fables-sandbox.mjs'), 'utf8').includes(JSON.stringify(computedRevision))
    , evidence_coverage_is_public_and_pinned: evidenceCoverage.corpus_revision === computedRevision && evidenceCoverage.overall.primary_source_incident_coverage === 1
    , false_safety_eval_is_pinned_and_passing: evalReport.corpus_revision === computedRevision && evalReport.false_safety.pass_rate === 1 && evalReport.negative_controls.false_positive_rate === 0
    , retrieval_metrics_are_pinned_and_passing: metricsReport.corpus_revision === computedRevision && metricsReport.discovery.recall_at_1 >= metricsReport.discovery.threshold && metricsReport.discovery.adversarial_recall_at_1 >= metricsReport.discovery.adversarial_recall_threshold
    , consumer_obligations_deny_authority: consumerObligations.authority === 'none' && consumerObligations.receipt_is_authorization === false
    , tool_call_hotpath_is_complete: fs.readdirSync(path.join(root, 'cards')).filter(file => /^AF-\d{4}\.json$/.test(file)).length === corpus.length && fs.readFileSync(path.join(root, 'INJECT.txt'), 'utf8').trim().split(/\s+/).length <= 80
  }
  return {
